 ...
 * Urutan di satu transaksi: kunci -> validasi -> tulis -> writeAudit -> onCalendarChanged.
...
async function schoolTarget(tx: Tx, scope: SchoolScope, ctx: ActionContext): Promise<HolidayTarget> {
...
const nationalTarget = (ctx: ActionContext): HolidayTarget => ({ schoolId: null, limitBackdate: false, today: wibDate(ctx.now) });
...
/** Kunci aplikasi mutasi libur satu sekolah, atau libur nasional (schoolId NULL). */
export const holidayLockKey = (schoolId: string | null): string => `holidays:${schoolId ?? "national"}`;

async function createHoliday(tx: Tx, target: HolidayTarget, input: CreateHolidayInput, ctx: ActionContext): Promise<HolidayDto> {
  await lockKey(tx, holidayLockKey(target.schoolId));
...
export function createSchoolHoliday(scope: SchoolScope, input: CreateHolidayInput, ctx: ActionContext): Promise<HolidayDto> {
  return withTx(async (tx) => createHoliday(tx, await schoolTarget(tx, scope, ctx), input, ctx));
}
...
export function updateSchoolHoliday(scope: SchoolScope, id: string, patch: UpdateHolidayInput, ctx: ActionContext): Promise<HolidayDto> {
  return withTx(async (tx) => updateHoliday(tx, await schoolTarget(tx, scope, ctx), id, patch, ctx));
}

export function deleteSchoolHoliday(scope: SchoolScope, id: string, ctx: ActionContext): Promise<{ id: string }> {
  return withTx(async (tx) => deleteHoliday(tx, await schoolTarget(tx, scope, ctx), id, ctx));
}
