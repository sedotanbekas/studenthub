...
test("semua job sudah diimplementasi (tidak ada lagi placeholder, termasuk push-dispatch)", () => {
  for (const name of JOB_NAMES) assert.notEqual(JOB_HANDLERS[name], placeholderHandler, name);
});
