...
function persona(key: string, label: string, caption: string, role: Role, name: string, extra: Partial<Identity> & Pick<DemoPersona, "className" | "todayRecord"> = {}): DemoPersona {
  const { className, todayRecord, ...identity } = extra;
  return {
    key, label, caption, className, todayRecord,
...
export const DEMO_PERSONAS: readonly DemoPersona[] = [
  ...
  persona("STUDENT", "Siswa · Alya", "X IPA 1 · belum absen", "STUDENT", "Alya Putri Ramadhani", { className: "X IPA 1" }),
  persona("STUDENT_BIMA", "Siswa · Bima", "X IPA 1 · sudah hadir", "STUDENT", "Bima Aditya Pratama", { className: "X IPA 1", todayRecord: { id: "demo-att-bima", status: "HADIR", source: "CHECKIN", checkInTimeLocal: "06:42", lateMinutes: null } }),
  persona("STUDENT_CITRA", "Siswa · Citra", "XI IPS 2 · terlambat", "STUDENT", "Citra Ayu Lestari", { className: "XI IPS 2", todayRecord: { id: "demo-att-citra", status: "TERLAMBAT", source: "CHECKIN", checkInTimeLocal: "07:31", lateMinutes: 31 } }),
