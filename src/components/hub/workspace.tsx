...
export function Workspace({ module }: { module: Module }) {
  ...
  return <div className="workspace-page"><div className="page-heading"><div><h1>{module.title}</h1><p>{module.description}</p></div><div className="heading-actions">{create && <button className="button primary" disabled={needsSchool} onClick={() => setAction({ op: create })}><Icon name="plus" size={17} />{create.id.startsWith("create") ? `Tambah ${module.key === "students" ? "siswa" : "baru"}` : actionTitle(create)}</button>}<OperationMenu available={available} onSelect={op => setAction({ op })} /></div></div>
