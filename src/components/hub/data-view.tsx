...
export function DataTable({ rows, onSelect, columns }: { rows: Row[]; onSelect?: (row: Row) => void; columns?: readonly string[] }) {
  const keys = columns?.filter(k => rows.some(r => k in r)) ?? Object.keys(rows[0] ?? {}).filter(k => !["id", "mustChangePassword"].includes(k) && !/Ids?$/.test(k)).slice(0, 6);
  return <div className="table-scroll"><table><thead><tr>{keys.map(k => <th key={k} scope="col">{label(k)}</th>)}{onSelect && <th scope="col"><span className="sr-only">Detail</span></th>}</tr></thead><tbody>{rows.map((row, i) => <tr key={String(row.id ?? i)}>{keys.map(k => <td key={k}><Cell name={k} value={row[k]} /></td>)}{onSelect && <td><button className="table-detail" onClick={() => onSelect(row)} aria-label={`Lihat detail ${display(row.name ?? row.title ?? row.student ?? row.termLabel ?? row.invoiceNo ?? i + 1)}`}><Icon name="arrow" size={17} /></button></td>}</tr>)}</tbody></table></div>;
}
